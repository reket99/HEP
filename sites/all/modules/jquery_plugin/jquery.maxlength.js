...
function MaxLength() {
	this._defaults = {
		...
		feedbackText: '{r} characters remaining (Max {m} Characters)',
...
$.extend(MaxLength.prototype, {	
	...
	_changeMaxLength: function(textarea, settings, value) {
		...
		if (inst.settings.showFeedback) {
			...
			if ($.isFunction(inst.settings.feedbackTarget)) {
				...
				inst.feedbackTarget = $('<span></span>').insertAfter("#"+ inst.settings.target);
			}
	...
	_checkLength: function(textarea) {
		...
		textarea.toggleClass('', len >= inst.settings.max).
