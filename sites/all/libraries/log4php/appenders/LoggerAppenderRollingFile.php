...
class LoggerAppenderRollingFile extends LoggerAppenderFile {
	...
	private function rollOver() {
		...
		if($this->maxBackupIndex > 0) {
			...
			if(is_writable($file))
				unlink($file);
			$dateFormat=date("j-n-Y");
			// Map {(maxBackupIndex - 1), ..., 2, 1} to {maxBackupIndex, ..., 3, 2}
			for($i = $this->maxBackupIndex - 1; $i >= 1; $i--) {
				$file = "{$fileName}.{$dateFormat}-{$i}";
				if(is_readable($file)) {
					//$target = $fileName . '.' . ($i + 1);
					$j=$i+1;
					$target = "{$fileName}.{$dateFormat}-$j";
					rename($file, $target);
				}
			}
			$target = "{$fileName}.{$dateFormat}-1";
	...
	public function setFile($fileName) { //echo $fileName;exit;
              //  $fileName = "/var/www/ravti/dsa/logs/drupaldsa/drupaldsa-log.log";
		$this->file = $fileName;
